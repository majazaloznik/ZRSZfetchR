...
vignette: >
  %\VignetteIndexEntry{zrszfetchr_workflow}
...
Presumably the filename/url will change with the new year, so the hardcoded url will need to be updated.
...
Steps 2.-3. are done manually for each new table and the old code is currently residing in the `.sandbox.R` file. 
Steps 4-6 are all run from a script in the `O:\Avtomatizacija\umar-automation-scripts` repository, triggered by an appropriate trigger in the Task scheduler. 
